...
case 15:return 14;
break;
case 16:this.begin('pre'); return 17;
break;
...
case 19:return 8;
break;
case 20:this.popState(); return 11;
break;
case 21:return 8;
break;
case 22:this.popState(); return 6;
break;
case 23:return 8;
break;
case 24:this.begin('p'); yy.lexer.unput(yy_.yytext); return 16;
break;
case 25:return 8;
break;
...
lexer.rules = [/^^\*\s+/,/^\n+/,/^$/,/^^\*\s+/,/^^#\s+/,/^\n+/,/^$/,/^^#\s+/,/^\(([^)]*)\)\[([^\]]*)\]/,/^([a-zA-Z0-9]+)(((\.|#)([^ (]*))+)?\(([^)]*)\)(\[([^\]]*)\])?/,/^([a-zA-Z0-9]+)(\[([^\]]*)\])/,/^^(h[1-6]|bq|blockquote)\s+/,/^\*([^*]*)\*/,/^_([^_]*)_\b/,/^`([^`]*)`/,/^\s\b/,/^<(pre|code)>/,/^<\/(pre|code)>/,/^./,/^\n+/,/^\n+/,/^./,/^\n+/,/^\n+/,/^^./,/^./,/^\n\b/,/^$/,/^$/];
lexer.conditions = {"b":{"rules":[8,9,10,12,13,14,20,21,26,27],"inclusive":false},"p":{"rules":[8,9,10,12,13,14,22,25,28],"inclusive":false},"pre":{"rules":[17,18,19],"inclusive":false},"li":{"rules":[1,2,3,8,9,10,12,13,14,21],"inclusive":false},"ol":{"rules":[5,6,7,8,9,10,12,13,14,21],"inclusive":false},"INITIAL":{"rules":[0,4,11,15,16,23,24,26,27],"inclusive":true}};return lexer;})()
parser.lexer = lexer;
return parser;
})();
